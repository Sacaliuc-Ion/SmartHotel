          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">{currentlyOccupied} of {rooms.length} rooms</p>
              </div>
                    ...
                    <div key={booking.id} className="p-4 border rounded-lg">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Room {room?.number} · {booking.guests} guest{booking.guests > 1 ? 's' : ''}</p>
                        </div>
                        <Badge variant={booking.paymentStatus === 'paid' ? 'default' : 'secondary'}>{booking.paymentStatus}</Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-gray-500">Until: {new Date(booking.checkOut).toLocaleDateString()}</p>

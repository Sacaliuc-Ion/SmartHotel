...
export const FrontDeskPage = () => {
  ...
  const occupancyRate = Math.round((currentlyOccupied / totalRooms) * 100);
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">{currentlyOccupied} of {totalRooms} rooms</p>
              </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardDescription>Departures Today</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            {arrivals.length === 0 ? (
              <p className="text-gray-500 text-center py-6">No arrivals scheduled for today</p>
            ) : (
              <div className="space-y-3">
                {arrivals.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="p-4 border rounded-lg hover:border-green-400 transition-colors"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h4 className="font-semibold text-gray-800">{booking.guestName}</h4>
                          <p className="text-sm text-gray-600">
                            Room {room?.number} • {booking.guests} guest{booking.guests > 1 ? 's' : ''}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-gray-500">
                          Stay: {new Date(booking.checkOut).toLocaleDateString()}
                        </p>
        ...
        <Card>
          ...
          <CardContent>
            {departures.length === 0 ? (
              <p className="text-gray-500 text-center py-6">No departures scheduled for today</p>
            ) : (
              <div className="space-y-3">
                {departures.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="p-4 border rounded-lg hover:border-orange-400 transition-colors"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h4 className="font-semibold text-gray-800">{booking.guestName}</h4>
